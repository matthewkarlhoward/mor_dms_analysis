...
PZM21_distances = {str(resi): {"pos": str(resi), "PZM21_distance": min_distance}
                    for resi, min_distance in zip(residue_numbers_mor, min_distances)
                    if min_distance < cutoff_distance}

# Save the results to a CSV file
csv_filename = "PZM21_distances_8EFO.csv"
with open(csv_filename, mode='w', newline='') as csv_file:
    fieldnames = ["pos", "PZM21_distance"]
    ...
    # Write the data
    for result in PZM21_distances.values():
