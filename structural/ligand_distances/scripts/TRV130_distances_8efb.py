...
TRV130_distances = {str(resi): {"pos": str(resi), "TRV130_distance": min_distance}
                    for resi, min_distance in zip(residue_numbers_mor, min_distances)
                    if min_distance < cutoff_distance}

# Save the results to a CSV file
csv_filename = "TRV130_distances_8efb.csv"
with open(csv_filename, mode='w', newline='') as csv_file:
    fieldnames = ["pos", "TRV130_distance"]
    ...
    # Write the data
    for result in TRV130_distances.values():
