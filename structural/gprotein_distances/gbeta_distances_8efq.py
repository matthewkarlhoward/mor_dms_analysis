...
Gbeta_distances = {str(resi): {"pos": str(resi), "Gbeta_distance": min_distance}
                    for resi, min_distance in zip(residue_numbers_mor, min_distances)
                    if min_distance < cutoff_distance}

# Save the results to a CSV file
csv_filename = "Gbeta_distances_8efq.csv"
with open(csv_filename, mode='w', newline='') as csv_file:
    fieldnames = ["pos", "Gbeta_distance"]
    ...
    # Write the data
    for result in Gbeta_distances.values():
